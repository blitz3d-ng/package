module Blitz3D
  module Tools
    module Config
      ...
      PLATFORMS = %w(win32 win64 macos linux).freeze
      ...
      def premake(mod)
        ...
        File.open(File.join(PREMAKE_DIR, "#{mod.id}.lua"), 'w') do |f|
          ...
          unless mod.platforms.empty?
            active_platforms = mod.platforms.map { |p| p == 'windows' ? ['win32', 'win64'] : p }.flatten
            inactive_platforms = PLATFORMS.select { |p| !active_platforms.include?(p) }.map(&:inspect)
      ...
      def run
        ...
        Runtime.all.each do |runtime|
          ...
          runtime.modules.each do |mod|
            f.write("BBMODULE_DECL( #{mod.parameterize.underscore} );\n")
          end

          f.write("\nvoid bbruntime_link( void (*link)( const char *sym,void *pc ) ){\n")
            runtime.modules.each do |mod|
              f.write("\t#{mod.parameterize.underscore}_link( link );\n")
            end
          f.write("}\n")
          ...
          write_create_calls = lambda do |i, l|
            (i + 1).times { f.write "\t" }
            id = runtime.modules[i].parameterize.underscore
            f.write("if( #{id}_create() ){\n")
            if i < runtime.modules.size - 1
              l.call(i + 1, l)
              (i + 1).times { f.write "\t" }
              f.write("\t#{id}_destroy();\n");
            else
          ...
          runtime.modules.reverse.each do |mod|
            f.write("\t#{mod.parameterize.underscore}_destroy();\n")
          end
          f.write("\treturn true;\n")
          f.write("}\n")

          runtime_stub_file = File.join(PREMAKE_DIR, "stubs/runtime.#{runtime.id}.cpp")

          if file_has_changed?(runtime_stub_file, f.string)
            File.open(runtime_stub_file, 'w') { |t| f.write f.string }
          end
          ...
          File.open(File.join(PREMAKE_DIR, "runtime.#{runtime.id}.lua"), 'w') do |f|
            ...
            f.write "  files \"stubs/runtime.#{runtime.id}.cpp\""

            f.write "  links 'stub'\n\n"

            # runtime.dependencies.each do |mod|
