      ...
      def runtimes
        Runtime.all.each do |runtime|
          ...
          runtime.platforms.each.with_index do |platform, i|
            ...
            static_target = "runtime.#{runtime.id}"
            shared_target = "#{static_target}.bin"

            f.write("#{'ELSE' if i > 0}IF(BB_#{platform.upcase})\n")
            f.write("  add_library(#{static_target} STATIC #{platform}.cpp)\n")
            f.write("  target_link_libraries(#{static_target} #{deps.join(' ')} ${SYSTEM_LIBS})\n") unless deps.empty?

            f.write("\n  add_library(#{shared_target} SHARED #{platform}.cpp)\n")
            f.write("  set_target_properties(#{shared_target} PROPERTIES OUTPUT_NAME \"#{static_target}\")\n") if  Module::METAPLATFORMS[:windows].include?(platform)
            f.write("  set_property(TARGET #{shared_target} APPEND_STRING PROPERTY LINK_FLAGS \" -static\")\n") if  Module::METAPLATFORMS[:windows].include?(platform)
            f.write("  target_link_libraries(#{shared_target} #{static_target})\n") unless deps.empty?
