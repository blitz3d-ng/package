      ...
      def premake(mod)
        ...
        File.open(File.join(PREMAKE_DIR, "#{mod.id}.lua"), 'w') do |f|
          ...
          unless mod.dependencies.nil? || mod.dependencies.empty?
            f.write "  links { "
            f.write mod.dependencies.map { |f| "'bb.#{f.id}'" }.join(', ')
            f.write " }\n\n"
          end
      ...
      def run
        ...
        Runtime.all.each do |runtime|
          ...
          runtime.platforms.each do |platform|
            ...
            FileUtils.mkdir_p File.join(LIB_DIR, platform)

            File.open(File.join(LIB_DIR, platform, "#{runtime.id}.config.json"), 'w') { |t| t.write JSON.pretty_generate(config) }
          ...
          File.open(File.join(PREMAKE_DIR, "runtime.#{runtime.id}.lua"), 'w') do |f|
            ...
            runtime.platforms.each do |platform|
              ...
              runtime.dependencies(:list).reverse.each do |mod|
                next unless mod.platforms.include?(platform)

                libs = mod.libraries(platform).map { |lib| "'#{lib}'" }
                f.write "    links { #{libs.join(', ')} }\n" unless libs.empty?
              end
            end
            f.write " filter {}\n"
