...
void main() {
  ...
  if( RS.UseVertexColor>0 ){
    bbVertex.Color = bbColor;
  }else{
    bbVertex.Color = RS.BrushColor;
  }
  ...
  if( RS.FullBright==0 ){
    ...
    vec3 LightPos=normalize( mat3( bbViewMatrix*bbLightMatrix0*rotationMatrix( vec3(1.0,0.0,0.0), 1.5708 ) )*vec3(0.0,1.0,0.0) );
    ...
    Ambient  += RS.Ambient;
    Diffuse  += vec4(1.0) * nDotVP;

    bbVertex.Diffuse=Diffuse.x;
    // Specular += gl_LightSource[i].specular * pf;

    bbVertex.Color = Ambient  * bbVertex.Color +
                     Diffuse  * bbVertex.Color;
    // color += Specular * gl_FrontMaterial.specular;
    bbVertex.Color = clamp( bbVertex.Color, 0.0, 1.0 );

    // bbVertex.Diffuse = 1.0;
  }
